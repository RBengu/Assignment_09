...
class FileIO:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        # try:
        file_CD = file_name[0]
        file_Track = file_name[1]
        with open(file_CD, 'w') as file:
            for disc in lst_Inventory:
                file.write(disc.get_record())
                
        saveDat = []
        with open(file_Track, 'a') as file:
            for disc in lst_Inventory:
                disc_id = disc.cd_id
                arrayLen = len(disc.cd_tracks)
                for row in range(arrayLen):
                    track_id = disc.cd_tracks[row].track_pos
                    track_tit = disc.cd_tracks[row].track_title
                    track_len = disc.cd_tracks[row].track_length
                    saveDat = ('{},{},{},{}\n'.format(disc_id, track_id, track_tit, track_len))
                    file.write(saveDat)
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            objFile = open(file_CD, 'a')
            objFile.close()
            
            objFile = open(file_Track, 'a')
            objFile.close()
            
            with open(file_CD, 'r') as file:
                for line in file:
                    data = line.strip().split(',')
                    row = DC.CD(data[0], data[1], data[2])
                    lst_Inventory.append(row)
                    
            with open(file_Track, 'r') as file:
                for line in file:
                    data = line.strip().split(',')
